...
internal class ChartUtils
{
  ...
  internal class func drawMultilineText(context context: CGContext?, text: String, knownTextSize: CGSize, point: CGPoint, align: NSTextAlignment, attributes: [String : AnyObject]?, constrainedToSize: CGSize, angle :Double)
  {
    ...
    let radAngle = CGFloat(M_PI * angle / 180.0)
    let c = CGFloat(cos(radAngle))
    let s = CGFloat(sin(radAngle))
    
    UIGraphicsPushContext(context)
    CGContextSaveGState(context)
    CGContextTranslateCTM(context,+rect.origin.x + s * rect.height , +rect.origin.y);
    CGContextRotateCTM(context, radAngle)
  ...
  internal class func drawMultilineText(context context: CGContext?, text: String, point: CGPoint, align: NSTextAlignment, attributes: [String : AnyObject]?, constrainedToSize: CGSize, angle :Double = 0)
  {
    
    let rect = text.boundingRectWithSize(constrainedToSize, options: .UsesLineFragmentOrigin, attributes: attributes, context: nil)
    drawMultilineText(context: context, text: text, knownTextSize: rect.size, point: point, align: align, attributes: attributes, constrainedToSize: constrainedToSize, angle :angle)
  }
